...
func (r *Ruleset) countRequiredBags(color string) int {
	count := 0
	color = strings.Replace(color, " ", "_", 1)
	// cycle rules in ruleset
	for _, rl := range r.ruleset {
		if rl.id == color {
			// cycle bags in rule
			for _, bgs := range rl.rules {
				n := bgs.quant
				k := r.countRequiredBags(bgs.color)
				count += n + n*k
...
func main() {
	r := Ruleset{}
	r.readRules("../input1.dat")
	fmt.Printf("%d idividual bags are required inside a single 'shiny gold' bags.\n",
		r.countRequiredBags("shiny gold"))
}
